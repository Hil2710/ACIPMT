...
def current_endpoints(apic, cookies, session):
    ...
    for x in range(0, payload_len):
        try:
            ...
            write_endpoints(session, dn, ip)
        except:
            print("Failed to lookup an endpoint for some reason.")
            pass


def write_endpoints(session, dn, ip):
...
def clean_endpoints(session):
    ...
    aged_eps = session.query(Endpoints).filter(Endpoints.last_seen < maxage).all()
    ...
    for x in range(0, ep_len):
        ...
        del_eps = session.query(Endpoints).filter(Endpoints.dn == aged_eps[x].dn).one()
...
# think this probably could be optimized? seems wasteful to ship off each individual
...
def query_hitcount(session, flow_data):
    ...
    try:
        flow = session.query(Flows).filter((Flows.src_ip == flow_data[0]) & 
                                           ...
                                           (Flows.protocol == flow_data[6])).one()
    ...
    if os.path.isfile(capfile):
        with open(capfile, 'r') as f:
            for line in f:
                ...
                # grab the hit count from above function for each individual flow
                hits = query_hitcount(session, flow_data)
                # if/elif/else to ensure that we have non-empty src/dst ip addresses
                # this just ensures that any capture stuff that saw l2 stuff doesnt get in our way
                if flow_data[0] == '':
                    continue
                elif flow_data[1] == '':
                    continue
                ...
                try:
                    flow = Flows(src_ip=flow_data[0], dst_ip=flow_data[1], tcp_src=flow_data[2], tcp_dst=flow_data[3], udp_src=flow_data[4], udp_dst=flow_data[5], protocol=flow_data[6], last_seen=current_time, hits=hits)
                    session.merge(flow)
                    session.commit()
                except:
                    print("Failed adding a flow to the DB.")
...
def build_contracts(session):
    # load up all flows into var "flow"
    ...
    for x in range(0, flow_len):
        ...
        contract = Contracts(src_epg=src_epg, dst_epg=dst_epg, tcp_src=tcp_src, tcp_dst=tcp_dst, udp_src=udp_src, udp_dst=udp_dst, protocol=protocol, last_seen=func.now())
...
def clean_tcp_contracts(session):
    tcp_ctrx = session.query(Contracts).filter(Contracts.protocol == 'tcp').all()   
                ...
                if tcp_ctrx[x].tcp_src == tcp_ctrx[y].tcp_dst:
                    ...
                    if (int(tcp_ctrx[x].tcp_src) > 1024 and
                        int(tcp_ctrx[y].tcp_src) > 1024):
                            print("High order port on source EPG, leaving in contract table. "
                                  "Issue is between following EPGs.")
                            print("Source EPG: " + tcp_ctrx[x].src_epg + ", Source TCP:" + tcp_ctrx[x].tcp_src + ", Destination TCP: " + tcp_ctrx[x].tcp_dst)
                            print("Destination EPG: " + tcp_ctrx[x].src_epg + ", Source TCP:" + tcp_ctrx[x].tcp_src + ", Destination TCP: " + tcp_ctrx[x].tcp_dst)
                            print("--------------")
                    else:
...
def clean_udp_contracts(session):
    udp_ctrx = session.query(Contracts).filter(Contracts.protocol == 'udp').all()   
    # Get quantity of all tcp contracts
                ...
                if udp_ctrx[x].udp_src == udp_ctrx[y].udp_dst:
                    ...
                    if (int(udp_ctrx[x].udp_src) > 1024 and
                        int(udp_ctrx[y].udp_src) > 1024):
                            print("High order port on source EPG, leaving in contract table. "
                                  "Issue is between following EPGs.")
                            print("Source EPG: " + udp_ctrx[x].src_epg + ", Source TCP:" + udp_ctrx[x].udp_src + ", Destination UDP: " + udp_ctrx[x].udp_dst)
                            print("Destination EPG: " + udp_ctrx[x].src_epg + ", Source TCP:" + udp_ctrx[x].udp_src + ", Destination UDP: " + udp_ctrx[x].udp_dst)
                            print("--------------")
                    else:
